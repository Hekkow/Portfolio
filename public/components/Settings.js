    ...
    template: `
      <div id="settings">
        <div id="settingsLeftPanel">
          <button class="settingsButton" v-for="tab in Object.values(data.settingsTabs)"
                  @click="data.openSettings = tab">{{ tab }}
          </button>
        ...
        <div id="settingsMainPanel">
          <div v-if="data.openSettings === data.settingsTabs.User" class="settingsTab">
            <settings-row class="bothSideRow">
              ...
              <div><input v-model="username">
                <button class="settingsButton" @click="changeUsername(username)">Change</button>
              </div>
            </settings-row>
            <settings-row class="bothSideRow">
              <label>Change password</label>
              <div><input type="password" v-model="password">
                <button class="settingsButton" @click="changePassword(password)">Change</button>
              </div>
            </settings-row>

            <settings-row>
              <button class="settingsButton" @click="logout()">Logout</button>
            </settings-row>
          ...
          <div v-if="data.openSettings === data.settingsTabs.Chats" class="settingsTab">
            <button class="settingsButton" v-if="!data.loadedUsers.get(data.userID).conversations.includes(3)"
